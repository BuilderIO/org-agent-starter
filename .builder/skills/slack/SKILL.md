...
### channel-history.mjs
Read recent messages from a channel. Accepts channel name or ID.
...
### thread-replies.mjs
Read all replies in a thread. Use the `ts` (timestamp) from channel-history output.
...
Read the full thread you're currently responding in. Extract the channel ID and thread timestamp from your `channelId` (format: `slack/thread/{teamId}/{channelId}/{threadTs}`). Useful when you need more context about the conversation that triggered your task.
...
### channel-info.mjs
Get details about a channel (purpose, topic, member count).
